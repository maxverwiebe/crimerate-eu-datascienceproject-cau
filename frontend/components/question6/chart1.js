...
const Question6Chart1 = () => {
    ...
    useEffect(() => {
        fetch("http://127.0.0.1:5000/api/question6/chart1")
            ...
            .then((json) => {
                ...
                setSelectedCountries(countries); // Setze als Standard alle Länder
            })
            .catch((error) => console.error("Error fetching data:", error));
    }, []);

    const formatData = () => {
        const countries = selectedCountries.length > 0 ? selectedCountries : Object.keys(chartData);
        const males = [];
        const females = [];
    ...
    const option = {
        ...
        tooltip: {
            ...
            formatter: (params) => {
                ...
                const maleValue = male.value === 0 ? "No data available" : Math.abs(male.value);
                const femaleValue = female.value === 0 ? "No data available" : female.value;
